...
lemma RGL_dual_cases: "g=RGL_Dual g1
  \<Longrightarrow> (\<exists>f. g1 = RGL_Test f \<Longrightarrow> P g)
  \<Longrightarrow> (\<exists>g3 g4. g1 = RGL_Choice (RGL_Dual g3) (RGL_Dual g4) \<Longrightarrow> P g)
  \<Longrightarrow> (\<exists>x g2. g1 = RGL_Rec x (RGL_Dual g2) \<Longrightarrow> P g)
  \<Longrightarrow> ((\<not>(\<exists>f. g1 = RGL_Test f)) \<Longrightarrow> (\<not>(\<exists> g3 g4. g1 = RGL_Choice (RGL_Dual g3) (RGL_Dual g4))) \<Longrightarrow>(\<not>(\<exists> x g2. g1 = RGL_Rec x (RGL_Dual g2)) \<Longrightarrow> P g))
  \<Longrightarrow> P g"
  by fastforce
...
lemma RGL_game_length_induct_std [case_names atm var strong]:
  "(\<And>a. P (RGL_Atm_Game a)) 
\<Longrightarrow> (\<And>x. P (RGL_Var x)) 
\<Longrightarrow> (\<And>n. (\<And>g. (\<And>m g1. m < n \<Longrightarrow> RGL_len_gm g1 < n \<Longrightarrow> P g1) \<Longrightarrow> RGL_len_gm g = n \<Longrightarrow> P g)) 
\<Longrightarrow> P g"
...
fun RGL_sy_dual :: " RGL_game \<Rightarrow> RGL_game" 
  ...
  | "RGL_sy_dual (RGL_Dual (RGL_Rec x (RGL_Dual g1))) = RGL_Rec x (RGL_sy_dual g1)"
...
(* basic observation about dsvar *)
lemma RGL_ds_var__Dual_ds_var:"RGL_ds_var g \<Longrightarrow> RGL_ds_var (RGL_Dual g)"
proof (induction g rule:RGL_ds_var.induct)
  case (single x)
  then show ?case
  by (simp add: RGL_ds_var.single RGL_ds_var.step)
next
  case (step g)
  then show ?case
  using RGL_ds_var.step by blast
qed

lemma RGL_ds_var_dual_free__ds_var: "RGL_ds_var g \<Longrightarrow> RGL_ds_var (RGL_dual_free x g)"
proof (induction g rule:RGL_ds_var.induct)
  case (single y)
  then show ?case 
  by (simp add: RGL_ds_var.single RGL_ds_var__Dual_ds_var)
next
  case (step g1)
  then show ?case
  by (simp add: RGL_ds_var__Dual_ds_var)
qed

lemma RGL_ds_var_syd__ds_var: "RGL_ds_var g \<Longrightarrow> RGL_ds_var (RGL_sy_dual g)"
proof (induction g rule:RGL_ds_var.induct)
  case (single y)
  then show ?case 
  by (simp add: RGL_ds_var.single RGL_ds_var__Dual_ds_var)
next
  case (step g1)
  then show ?case
  by (metis RGL_ds_var.simps RGL_sy_dual.simps(7,8))
qed
...
lemma RGL_dsnml__dsvar: "RGL_dsnml_gm g \<Longrightarrow> g = RGL_Dual g1
...
lemma RGL_dsnml_dual_cases: "RGL_dsnml_gm g \<Longrightarrow> g=RGL_Dual g1
  \<Longrightarrow> (\<exists>f. g1 = RGL_Test f \<Longrightarrow> P g)
  \<Longrightarrow> (\<exists>g3 g4. g1 = RGL_Choice (RGL_Dual g3) (RGL_Dual g4) \<Longrightarrow> P g)
  \<Longrightarrow> (\<exists>x g2. g1 = RGL_Rec x (RGL_Dual g2) \<Longrightarrow> P g)
  \<Longrightarrow> (RGL_ds_var g1 \<Longrightarrow> P g)
  \<Longrightarrow> P g"
  using RGL_dsnml__dsvar RGL_DChoice_def RGL_DRec_def RGL_DTest_def by fastforce
...
lemma RGL_normalize__dsnml: "RGL_dsnml_gm (RGL_normalize_game g)"
proof (induction g rule:RGL_game_length_induct_std)
  case (atm a)
  then show ?case
  by (simp add: dsnml_atm)
next
  case (var x)
  then show ?case
  by (simp add: dsnml_dsvar single)
next
  case (strong n g)
  then show ?case
  proof (cases g)
    case (RGL_Atm_Game x1)
    then show ?thesis
    by (simp add: dsnml_atm)
  next
    case (RGL_Var x2)
  ...
  next
    case (RGL_Dual g1)
    then show ?thesis (* When g = Dual g1, g1 is dsnml, then g1 must be of certain forms. *)
    proof (rule RGL_dual_cases[of "g""g1" "\<lambda>g. RGL_dsnml_gm (RGL_normalize_game g)"])
      from strong.IH[of "RGL_len_gm g1""g1"] strong.hyps RGL_Dual have a0:"RGL_dsnml_gm (RGL_normalize_game g1)" by auto
      then show "\<exists>f. g1 = RGL_Test f \<Longrightarrow> RGL_dsnml_gm (RGL_normalize_game g)" apply (simp add:RGL_Dual)
      proof - assume a1:"\<exists>f. g1 = RGL_Test f" and a2:"RGL_dsnml_gm (RGL_normalize_game g1)"
        from a1 obtain f where "g1=RGL_Test f" by auto
        then show "RGL_dsnml_gm (RGL_normalize_game (RGL_Dual g1))" using a0
...
(* This function reduces Dual (Dual x) for all x in a game g. 
  DTest = Dual (Test .)
  DChoice = Dual (Choice (Dual .) (Dual .))
  DRec x g = Dual (Rec x (dualx g)) 
  
  pattern matching for Dual can safely reduce Dual Dual.
*)
fun RGL_red_dd:: "RGL_game \<Rightarrow> RGL_game"
and RGL_red_dd_fml:: "RGL_fml \<Rightarrow> RGL_fml" where
  "RGL_red_dd (RGL_Atm_Game a) = RGL_Atm_Game a"
| "RGL_red_dd (RGL_Var x) = (RGL_Var x)"
| "RGL_red_dd (RGL_Dual (RGL_Dual (RGL_Var x))) = RGL_Var x"
| "RGL_red_dd (RGL_Dual g) = RGL_Dual (RGL_red_dd g)"
| "RGL_red_dd (RGL_Test f) = RGL_Test (RGL_red_dd_fml f)"
| "RGL_red_dd (RGL_Choice g1 g2) = RGL_Choice (RGL_red_dd g1) (RGL_red_dd g2)"
| "RGL_red_dd (RGL_Seq g1 g2) = RGL_Seq (RGL_red_dd g1) (RGL_red_dd g2)"
| "RGL_red_dd (RGL_Rec x g) = RGL_Rec x (RGL_red_dd g)"
| "RGL_red_dd_fml (RGL_Atm_fml a) = (RGL_Atm_fml a)"
| "RGL_red_dd_fml (RGL_Not f) = RGL_Not (RGL_red_dd_fml f)"
| "RGL_red_dd_fml (RGL_Or f1 f2) = RGL_Or (RGL_red_dd_fml f1) (RGL_red_dd_fml f2)"
| "RGL_red_dd_fml (RGL_Mod g f) = RGL_Mod (RGL_red_dd g) (RGL_red_dd_fml f)"

definition RGL_gm_equiv :: "RGL_game \<Rightarrow> RGL_game \<Rightarrow> bool" (infix "\<simeq>\<^sub>g" 50)
  where
 "RGL_gm_equiv g1 g2 \<equiv> RGL_red_dd g1 = RGL_red_dd g2"

definition RGL_fml_equiv:: "RGL_fml \<Rightarrow> RGL_fml \<Rightarrow> bool" (infix "\<simeq>\<^sub>f" 50)
  where "RGL_fml_equiv f1 f2 \<equiv> RGL_red_dd_fml f1 = RGL_red_dd_fml f2"
...
definition sym:: "('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> bool" where
  "sym R \<equiv> \<forall> a b. R a b \<longleftrightarrow> R b a"

lemma sym_star__sym: "sym R \<Longrightarrow> sym (star R)"
  unfolding sym_def 
  apply rule
proof fix a b assume ass:"\<forall>a b. R a b = R b a"
  show "star R a b = star R b a" apply auto
  proof - show "star R a b \<Longrightarrow> star R b a"
      thm star.induct[where P="\<lambda> a b. star R b a"]
    proof (induction rule:star.induct)
      case (refl x)
      then show ?case using star.refl by auto
    next
      case (step x y z)
      then show ?case using ass star_step1
      by (metis star_trans)
  qed
next
  show "star R b a \<Longrightarrow> star R a b"
  proof - show "star R b a \<Longrightarrow> star R a b"
      thm star.induct[where P="\<lambda> a b. star R b a"]
    proof (induction rule:star.induct)
      case (refl x)
      then show ?case using star.refl by auto
...
lemma RGL_ddrel_sym: fixes g1::"RGL_game" and g2::"RGL_game" and f1::"RGL_fml" and f2::"RGL_fml"
  shows "(RGL_ddrel_gm g1 g2 \<longrightarrow> RGL_ddrel_gm g2 g1) \<and> (RGL_ddrel_fml f1 f2 \<longrightarrow> RGL_ddrel_fml f2 f1)"
proof (induction rule:RGL_ddrel_gm_RGL_ddrel_fml.induct[where ?x1.0="g1"])
  case (RGL_ddgm_varl x)
  then show ?case using RGL_ddgm_varr by auto
next
  case (RGL_ddgm_varr x)
  then show ?case using RGL_ddgm_varl by auto
next
  case (RGL_ddgm_choi g1 g2 g3 g4)
  then show ?case using Syntax.RGL_ddgm_choi by auto
next
  case (RGL_ddgm_seq g1 g2 g3 g4)
  then show ?case using Syntax.RGL_ddgm_seq by auto
next
  case (RGL_ddgm_dchoi g1 g2 g3 g4)
  then show ?case using Syntax.RGL_ddgm_dchoi by auto
next
  case (RGL_ddgm_rec g1 g2 x)
  then show ?case using Syntax.RGL_ddgm_rec by auto
next
  case (RGL_ddgm_drec g1 g2 x)
  then show ?case using Syntax.RGL_ddgm_drec by auto
next
  case (RGL_ddgm_tst f1 f2)
  then show ?case using Syntax.RGL_ddgm_tst by auto
next
  case (RGL_ddgm_dtst f1 f2)
  then show ?case using Syntax.RGL_ddgm_dtst by auto
next
  case (RGL_ddfml_not f1 f2)
  then show ?case using Syntax.RGL_ddfml_not by auto
next
  case (RGL_ddfml_or f1 f2 f3 f4)
  then show ?case using Syntax.RGL_ddfml_or by auto
next
  case (RGL_ddfml_mod g1 g2 f1 f2)
  then show ?case using Syntax.RGL_ddfml_mod by auto
qed

lemma RGL_ddrel_rt_sym: "(g1 \<sim>\<^sub>g g2 \<longrightarrow> g2 \<sim>\<^sub>g g1) \<and> (f1 \<sim>\<^sub>f f2 \<longrightarrow> f2 \<sim>\<^sub>f f1)"
  using RGL_ddrel_sym sym_star__sym
  by (metis RGL_ddrel_fml_rt_def RGL_ddrel_gm_rt_def Syntax.sym_def)
...
next
  ...
  then show ?case apply simp
    using RGL_dual_cases 
  proof - 
    case True
    then show ?thesis
    proof fix y assume "g1=RGL_Var y"
      then show "RGL_dual_free x (RGL_dual_free x (RGL_Dual g1)) \<sim>\<^sub>g RGL_Dual g1"
    qed
  next
    case False
    then show ?thesis
...
(* dual_free preserves dsnml because it goes through each pattern *)
lemma RGL_dsnml_dual_free__dsnml:
  fixes g::"RGL_game" and f::"RGL_fml"
  shows "(RGL_dsnml_gm g \<longrightarrow> RGL_dsnml_gm (RGL_dual_free x g)) \<and> (RGL_dsnml_fml f \<longrightarrow> RGL_dsnml_fml (RGL_dual_free_fml x f))"
proof (induction rule:RGL_dsnml_gm_RGL_dsnml_fml.induct[where ?x1.0="g" and ?x2.0="f"])
  case (dsnml_atm a)
  then show ?case
  by (simp add: RGL_dsnml_gm_RGL_dsnml_fml.dsnml_atm)
next
  case (dsnml_dsvar g1)
  then show ?case
  proof (induction g1 rule:RGL_ds_var.induct)
    case (single z)
    then show ?case
      by (simp add: RGL_ds_var.single RGL_ds_var.step RGL_dsnml_gm_RGL_dsnml_fml.dsnml_dsvar)
  next
    case (step g2)
    then show ?case using RGL_ds_var_dual_free__ds_var RGL_ds_var__Dual_ds_var
     RGL_dsnml_gm_RGL_dsnml_fml.dsnml_dsvar by force
  qed
next
  case (dsnml_tt f)
  then show ?case 
  by (simp add: RGL_dsnml_gm_RGL_dsnml_fml.dsnml_tt)
next
  case (dsnml_dtt f)
  then show ?case 
  using RGL_DTest_def RGL_dsnml_gm_RGL_dsnml_fml.dsnml_dtt RGL_dual_free.simps(2,7) by presburger
next
  case (dsnml_choi g1 g2)
  then show ?case 
  using RGL_dsnml_gm_RGL_dsnml_fml.dsnml_choi by force
next
  case (dsnml_dchoi g1 g2)
  then show ?case 
  using RGL_DChoice_def RGL_dsnml_gm_RGL_dsnml_fml.dsnml_dchoi by auto
next
  case (dsnml_seq g1 g2)
  then show ?case 
  by (simp add: RGL_dsnml_gm_RGL_dsnml_fml.dsnml_seq)
next
  case (dsnml_rec g1 y)
  then show ?case 
  by (simp add: RGL_dsnml_gm_RGL_dsnml_fml.dsnml_rec)
next
  case (dsnml_drec g1 y)
  then show ?case
  using RGL_DRec_def RGL_dsnml_gm_RGL_dsnml_fml.dsnml_drec RGL_dual_free_vars_comm(1) by fastforce
next
  case (dsnml_atmf a)
  then show ?case
  by (simp add: RGL_dsnml_gm_RGL_dsnml_fml.dsnml_atmf)
next
  case (dsnml_notatm a)
  then show ?case
  by (simp add: RGL_nml_Not_atm RGL_nml__dsnml(2))
next
  case (dsnml_or f1 f2)
  then show ?case
  by (simp add: RGL_dsnml_gm_RGL_dsnml_fml.dsnml_or)
next
  case (dsnml_and f1 f2)
  then show ?case
  using RGL_And_def RGL_dsnml_gm_RGL_dsnml_fml.dsnml_and RGL_dual_free_fml.simps(2,3) by presburger
next
  case (dsnml_mod g1 f1)
  then show ?case
  using RGL_dsnml_gm_RGL_dsnml_fml.dsnml_mod by force
qed

(* syntactic dual preserves dsnml because of its deep pattern matching *)
lemma RGL_dsnml_syd__dsnml: 
  fixes g::"RGL_game" and f::"RGL_fml"
  shows
  "(RGL_dsnml_gm g \<longrightarrow> RGL_dsnml_gm (RGL_sy_dual g)) \<and> (RGL_dsnml_fml f \<longrightarrow> RGL_dsnml_fml (RGL_sy_comp f))"
  thm RGL_dsnml_gm_RGL_dsnml_fml.induct[where ?x1.0="g" and ?x2.0="f"]
proof (induction rule:RGL_dsnml_gm_RGL_dsnml_fml.induct[where ?x1.0="g" and ?x2.0="f"])
  case (dsnml_atm a)
  then show ?case 
    by (metis RGL_dsnml_gm_RGL_dsnml_fml.dsnml_atm RGL_sy_dual.simps(1,2) atm_gm.exhaust)
next
  case (dsnml_dsvar g)
  then show ?case
  by (metis RGL_ds_var.simps RGL_dsnml_gm_RGL_dsnml_fml.dsnml_dsvar RGL_sy_dual.simps(3,7,8))
next
  case (dsnml_tt f)
  then show ?case
  by (simp add: dsnml_dtt)
next
  case (dsnml_dtt f)
  then show ?case
  by (simp add: RGL_DTest_def dsnml_tt)
next
  case (dsnml_choi g1 g2)
  then show ?case
  by (simp add: dsnml_dchoi)
next
  case (dsnml_dchoi g1 g2)
  then show ?case
  by (simp add: RGL_DChoice_def dsnml_choi)
next
  case (dsnml_seq g1 g2)
  then show ?case
  using RGL_dsnml_gm_RGL_dsnml_fml.dsnml_seq by force
next
  case (dsnml_rec g1 x)
  then show ?case using RGL_dsnml_dual_free__dsnml[of "RGL_sy_dual g1"]
  using dsnml_drec by force
next
  case (dsnml_drec g x)
  then show ?case sorry
next
  case (dsnml_atmf a)
  then show ?case 
    by (simp add: dsnml_notatm) 
next
  case (dsnml_notatm a)
  then show ?case 
    by (simp add: dsnml_atmf)
next
  case (dsnml_or f1 f2)
  then show ?case 
    using dsnml_and by force
next
  case (dsnml_and f1 f2)
  then show ?case 
    by (simp add: RGL_And_def dsnml_or)
next
  case (dsnml_mod g f)
  then show ?case
    by (simp add: RGL_dsnml_gm_RGL_dsnml_fml.dsnml_mod)
qed
...
proof (induction g rule:RGL_nml_game_induct)
  case (rGL_nml_Atm_Game a)
  then show ?case by (cases a) (auto simp add:RGL_ddnml_Atm_game)
next
  case (rGL_nml_Var y)
  then show ?case
  proof (cases "x=y")
    case True
    then show ?thesis
      using RGL_nml__ddnml RGL_ddnml_Var RGL_ddnml_DDVar by auto
  next
    case False
    then show ?thesis using RGL_ddnml_DVar by auto
  qed
next
  case (rGL_nml_DVar y)
  then show ?case 
  proof (cases "x=y")
    case True
    then show ?thesis using RGL_nml__ddnml RGL_nml_DVar by auto
  next
    case False
    then show ?thesis
      using RGL_ddnml_Var by auto
  qed  
next
  case (rGL_nml_Test f)
  then show ?case
  proof (simp)
    from rGL_nml_Test.prems have a:"RGL_nml_fml f"
    using RGL_DChoice_def RGL_DRec_def RGL_DTest_def RGL_game.distinct(33,35) RGL_game.inject(4) RGL_game.simps(13,23) RGL_nml_game.cases by fastforce
    from rGL_nml_Test.prems have b:"RGL_fml_closed f"
    by (smt (verit, ccfv_SIG) RGL_DChoice_def RGL_DRec_def RGL_DTest_def RGL_fml_closed_def RGL_game.distinct(23,31,33,35) RGL_game.simps(13,23) RGL_nml_game.simps
        free_var_game.simps(4))
  then have a1:"RGL_dual_free_fml x f =f" using RGL_closed_dual_free_id(2) by auto

  from b have b1:"RGL_game_closed (RGL_DTest f)" using RGL_fml_closed__DTest_closed_iff by auto
  then have "RGL_dual_free x (RGL_DTest f) = RGL_DTest f" using RGL_closed_dual_free_id by auto
  then
    show "RGL_ddnml_game (RGL_DTest (RGL_dual_free_fml x f)) \<and> RGL_ddnml_game (RGL_dual_free x (RGL_DTest f))" 
      using RGL_ddnml_DTest[of "f"] a a1 b RGL_nml__ddnml by auto
  qed
next
  case (rGL_nml_DTest f)
  then show ?case unfolding RGL_DTest_def
  proof (simp)
    from rGL_nml_DTest.prems have a:"RGL_nml_fml f"
    using RGL_DChoice_def RGL_DRec_def RGL_DTest_def RGL_game.distinct(33,35) RGL_game.inject(4) RGL_game.simps(13,23) RGL_nml_game.cases by fastforce
    from rGL_nml_DTest.prems have b:"RGL_fml_closed f"
    by (smt (verit, ccfv_SIG) RGL_DChoice_def RGL_DRec_def RGL_DTest_def RGL_fml_closed__DTest_closed_iff RGL_fml_closed__test_closed_iff RGL_game.distinct(15,3,31,35)
        RGL_game.inject(3) RGL_game.simps(21,33,35,37) RGL_nml_game.simps)
    then have a1:"RGL_dual_free_fml x f =f" using RGL_closed_dual_free_id(2) by auto
  
    show "RGL_ddnml_game (RGL_Test (RGL_dual_free_fml x f))"
      using RGL_ddnml_Test[of "f"] a1 b a RGL_nml__ddnml by auto
  qed
next
  case (rGL_nml_Choice g1 g2)
  then show ?case sorry
...
next
  ...
  then show ?case sorry
next
  case (rGL_nml_Rec y g)
  then show ?case
  proof (cases "x=y")
    case True
    then show ?thesis
    proof (simp)
      from rGL_nml_Rec.prems have "RGL_nml_game g"
        using rGL_nml_Rec.hyps by force
      then have IH:"RGL_ddnml_game (RGL_sy_dual (RGL_dual_free x g)) \<and> RGL_ddnml_game (RGL_dual_free x (RGL_sy_dual g))" using rGL_nml_Rec.IH by simp

      from IH True have "RGL_ddnml_game (RGL_dual_free y (RGL_sy_dual g))" by auto
      then have r1:"RGL_ddnml_game (RGL_DRec y (RGL_dual_free y (RGL_sy_dual g)))" using RGL_ddnml_DRec by auto

      have s1:"RGL_dual_free y (RGL_DRec y (RGL_dual_free y (RGL_sy_dual g))) = RGL_DRec y (RGL_dual_free y (RGL_sy_dual g))" unfolding RGL_DRec_def by simp
      have s2:"RGL_ddnml_game (RGL_DRec y (RGL_dual_free y (RGL_sy_dual g)))" using IH True RGL_ddnml_DRec by auto

      from s1 s2 have r2:"RGL_ddnml_game (RGL_dual_free y (RGL_DRec y (RGL_dual_free y (RGL_sy_dual g))))" by auto
      show "RGL_ddnml_game (RGL_DRec y (RGL_dual_free y (RGL_sy_dual g))) \<and> RGL_ddnml_game (RGL_dual_free y (RGL_DRec y (RGL_dual_free y (RGL_sy_dual g))))"
        using r1 r2 by simp
    qed
  next
    case False
    then show ?thesis
    proof (simp)
      (* We want to express that y carries at most two Duals. *)
      show "RGL_ddnml_game (RGL_DRec y (RGL_dual_free y (RGL_sy_dual (RGL_dual_free x g)))) \<and> RGL_ddnml_game (RGL_dual_free x (RGL_DRec y (RGL_dual_free y (RGL_sy_dual g))))"
    qed
  qed
  qed
next
